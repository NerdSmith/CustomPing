...
std::map<std::uint32_t, PingConfig> Ping::Exec() {
  ...
  //         // printBytes(recvPkg);
  //         // std::cout << "|" << inet_ntoa(addrToSockAddr[currIP].sin_addr)
  //         << "|" << fromlen << std::endl;
  //         // TODO: resolve
  //         if (recvfrom(
  //                 addrToSock[currIP],
  ...
  //             std::endl; this->addrCfgs[*((uint32_t*)
  //             recvPkg.ping_pkg.msg)].status = PingStatus::OK;
  //         }
  ...
  if (sendto(sock, &pkg, sizeof(pkg), 0,
  ...
  //     printBytes(recvPkg);
  //     std::cout << "ansv: " << *((uint32_t*) recvPkg.ping_pkg.msg) <<
  //     std::endl; this->addrCfgs[*((uint32_t*) recvPkg.ping_pkg.msg)].status =
  //     PingStatus::OK;
  // }
  ...
  //     for (std::set<uint32_t>::iterator it = toSend.begin(); it !=
  //     toSend.end(); it++) {
